...
панели, сохранение позиции, авто-снятие по простою (2 мин). 71 тест зелёный.

### Проверено вживую (работает)
- LEFT / RIGHT / DOUBLE клики; DRAG (выделение/область)
- MIDDLE: auto-scroll (якорь, динамика, intensity=0.5, dwell→ЛКМ+выход,
  мазок по панели→выход) + умный клик по ссылке → новая вкладка (Safari/Firefox)
