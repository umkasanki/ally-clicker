...
- **Стабильная подпись C1 — СДЕЛАНО.** Отдельный keychain `allyclicker.keychain-db`
  (пароль `allyclicker`), self-signed сертификат "AllyClicker Self-Signed".
  `App/setup-signing.sh` создаёт его (можно по SSH), `build-app.sh` подписывает им.
  Подпись/сборка идут по SSH без интерактива. Accessibility выдан и **привязан к
  стабильной подписи** — при пересборке больше не слетает.
- **НЕ ПРОВЕРЕНО: реальный клик и Y-flip координат.** Приложение запущено, доступ
  есть. Следующий шаг — тест: зарядить LEFT, замереть на десктопе, проверить что
  клик срабатывает И попадает точно под курсор (не смещён по вертикали).
- Если Y-flip неверен → чинить `ScreenGeometry`/`CGMouseInjector` (перенос между
  bottom-left AppKit и top-left CGEvent).
- Cert-файлы в `/tmp` эфемерны (чистятся при перезагрузке) — если keychain уже
  настроен, они не нужны; для пере-настройки пересоздать (см. историю).
