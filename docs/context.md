...
### ⏳ Сделать, когда Мак включён (сейчас выключен)
- **Собрать/проверить** незакоммиченно-протестированную правку `c8eb48d` — звук
  фидбэка на СТАРТЕ драга (mouseDown), а не только на завершении. `swift test` +
  `App/install.sh` + проверить на слух.
- **Стабильная подпись релизов (вариант B — делаю я):** экспортировать локальную
  self-signed идентичность `AllyClicker Self-Signed` в `.p12`, залить два секрета в
  `umkasanki/ally-clicker`: `SIGNING_P12_BASE64` (base64 p12) и `SIGNING_P12_PASSWORD`.
  Workflow (`release.yml`) уже импортирует их опционально. Тогда грант Универсального
  доступа держится при всех обновлениях. (Экспорт приватного ключа блокируется
  auto-режимом — потребуется подтвердить.)
- **Первый грант Универсального доступа** для текущей self-signed сборки всё ещё
  нужно выдать вручную один раз (тумблер в Универсальном доступе; замкнутый круг —
  до гранта приложение кликать не может; мышь/помощник/встроенный Dwell).
- После сборки — при желании выпустить **v0.1.3** (звук старта драга) тегом.
