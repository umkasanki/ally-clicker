...
⚠️ **`AllyClicker.App` собран, но локально не проверен и не может быть проверен:**
apt-версия .NET SDK не содержит `Microsoft.NET.Sdk.WindowsDesktop`. Первый зелёный
прогон `windows-ci.yml` — единственное подтверждение, что WPF-проект вообще валиден.
...
- Подпись **Authenticode** доверенным сертификатом — **обязательна** (не «иначе SmartScreen
  поворчит»). Сертификат платный; это блокирующая закупка, а не опция.
...
3. **Сертификат для подписи** (W7) — платный, и теперь **блокирующий**: без него нельзя
   включить `uiAccess`, а без `uiAccess` UIPI молча глотает клики по окнам, запущенным от
   администратора (`docs/spec.md` §3.6). Это уже не «предупреждение SmartScreen», как
   казалось до ревью W0, и не аналог отложенной нотаризации на macOS — там неподписанное
   приложение всё же работает, здесь функциональность режется. Решать до W7, т.к. закупка
   сертификата имеет собственный срок.
