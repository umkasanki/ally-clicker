...
#### ✅ Settings + golden-харнесс — СДЕЛАНО (2026-08-01)
Портированы `Settings` (все 8 секций), `KeyboardTarget`, `IconStyle`, `Orientation`.

**Ключевое решение — дифференциальная проверка вместо ручного переноса ассертов.**
Добавлена цель `macos/Sources/SettingsGolden` (dev-инструмент, в приложение не входит):
она печатает, как **эталонная реализация** декодирует набор документов. Вывод закоммичен
как `windows/AllyClicker.Core.Tests/Fixtures/settings-golden.json` (28 кейсов), C#-тест
сверяется с ним структурно. Перегенерация — **без Мака**:
```
...
```
Смысл: ручной перенос ассертов проверяет лишь то, что автор Swift-тестов догадался
проверить, и одинаково охотно соглашается и с истиной, и с неверным прочтением.
Golden фиксирует фактическое поведение — например, `effectiveDwellMouseSeconds = 0.001`
в кейсе `calibration-enabled` (формула с неподобранным `multiplier` упирается в нижний
кламп 1 мс). Такой ассерт руками не написал бы никто.

**Проверено мутациями**, что набор умеет краснеть: сдвиг нижнего клампа `intensity`
0.05→0.10 и отключение всплытия ON/OFF первым — оба падают с указанием кейса и поля.

**Главное расхождение платформ, ради которого декодер написан руками:** Swift трактует
явный `null` как отсутствующий ключ и берёт дефолт, а `System.Text.Json` на
non-nullable типе бросает исключение. `settings.json` с `null` уронил бы приложение на
старте — для пользователя, у которого это единственный способ управления, невосстановимо.
Отсюда `Json.cs` с семантикой `decodeIfPresent`. Кейс `explicit-null-falls-back` его держит.

**Отступления от 1:1 и почему:**
- Типы секций вынесены на верхний уровень (`Timing`, `Clicks`, …), а не вложены в
  `Settings`: в C# свойство не может называться так же, как вложенный тип, тогда как в
  Swift свойство со строчной буквы и конфликта нет.
- `Panel` → **`PanelSettings`**, иначе столкновение с `Zone.Panel`.
- Записи **иммутабельные**, правки через `with`. Swift-структуры копируются при
  присваивании; мутабельный C#-класс дал бы «копию», разделяющую состояние с оригиналом,
  и правка в окне настроек протекла бы в работающий движок до нажатия Apply.
  ⚠️ Ревью показало, что первая версия эту гарантию **только декларировала**:
  `IReadOnlyList` лишь прячет мутирующие методы, не убирая их, поэтому `DefaultItems`
  приводился обратно к массиву и был доступен на запись — статический, общий на процесс.
  Исправлено: `Array.AsReadOnly` + копия во входе `Items` + обёртка на выходе
  `Normalize` (её будет звать редактор панели в W5). Урок общий: тип-интерфейс не
  делает данные неизменяемыми, неизменяемым делает владение хранилищем.
- `PanelSettings` имеет собственные `Equals`/`GetHashCode`: записи сравнивают
  ссылочные поля по ссылке, из-за чего два одинаковых списка кнопок считались бы разными.

⚠️ **Не покрыто golden:** поведение при **неверном типе** значения (`"width": "abc"`).
Swift там бросает `typeMismatch`, порт повторяет это по прочтению исходника, но фикстура
содержит только успешно декодируемые документы. Если понадобится строгая гарантия —
расширить дампер секцией падающих кейсов.
...
### Где мы стоим (на 2026-08-01, конец сессии)
- **W0 — закрыт.** Каркас `windows/` + `windows-ci.yml`, CI зелёный на обеих джобах.
- **W1 — идёт.** Готовы: `Point`, `ClickAction`, `PanelCommand`, `Zone`, `Effect`,
  `PanelItem`, `Ports`, `Settings` (+ `KeyboardTarget`, `IconStyle`, `Orientation`),
  golden-харнесс. **66 тестов зелёные.**
- **Осталось по W1:** `DwellEngine` (353 стр.), `AutoScrollEngine`, `AutoScrollController`,
  `DwellController`, `SettingsStore`. Плюс порт остальных тест-файлов из
  `macos/Tests/AllyClickerTests/`.
- Ветка `feature/windows-app`, всё запушено, `main` остаётся релизным для macOS.

### Первые команды
```bash
git checkout feature/windows-app
dotnet test windows/AllyClicker.Core.slnf          # наш порт: должно быть 66/66
docker run --rm -v "$PWD/macos":/pkg -w /pkg swift:6.0 swift test   # эталон: 75/75
```
Мак не нужен ни для того, ни для другого. `dotnet` — apt-версия 8.0.129; она **не** умеет
собирать `AllyClicker.App` (нет `Microsoft.NET.Sdk.WindowsDesktop`), поэтому всё, что
трогает WPF, проверяется только в CI.

### Следующий шаг — `DwellEngine`
Порядок, установленный опытом этой сессии: **сначала снять со Swift golden-трассу**, потом
писать C# под неё. Для стейт-машины это работает лучше, чем для настроек: расхождение может
быть не в значении, а в **порядке эффектов внутри тика**, и глазами такое не находится.
Расширить `macos/Sources/SettingsGolden` (или добавить рядом вторую цель) так, чтобы она
прогоняла скриптованную последовательность тиков и печатала `[Effect]` покадрово.

Особое внимание — границам: swipe-debounce 15 мс, re-fire gate, idle-disarm. Там `>` вместо
`>=` даёт расхождение, которое ловится только трассой. И двухфазный драг — единственная
часть с настоящим жизненным циклом: залипшая кнопка означает, что пользователь потерял
управление мышью.

### Открытый вопрос, решить в начале сессии
Закрывать ли пробел по **неверному типу** значения (`"width": "abc"`): Swift бросает
`typeMismatch`, порт это повторяет, но по прочтению исходника — фикстурой не прибито, т.к.
дампер собирает только успешно декодируемые документы. Лечится второй секцией падающих
кейсов, ~20 минут. Либо сразу браться за `DwellEngine`, а это оставить на потом.

### Правила, которые не переоткрывать
- Эталон поведения — Swift в `macos/Sources/AllyClickerCore/`. **При расхождении прав он**
  (проверен вживую пользователем).
- Ручной перенос ассертов проверяет лишь то, что автор Swift-тестов догадался проверить.
  Golden-фикстура фиксирует фактическое поведение — предпочитать её.
- Зелёный набор обязан уметь краснеть: после добавления фикстуры прогнать мутацию.
- `IReadOnlyList` не делает данные неизменяемыми. Владение хранилищем — делает.
- Swift трактует явный `null` как отсутствующий ключ; `System.Text.Json` бросает.
