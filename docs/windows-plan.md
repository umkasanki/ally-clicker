...
спайк не пойдёт, соблазн «да просто запросим права администратора» — тупик, а не обход:
такое сочетание Windows не принимает.
...
- **Нужен публично доверенный сертификат** → закупку начинать немедленно, не в W7.
  Кандидат — **Certum Open Source**: выдаётся физлицам, требует открытого проекта, им
  подписан OptiKey (ассистивный проект той же ниши). Цена зависит от варианта, и источники
  расходятся — ориентир: **облачный ~€60, аппаратный (смарт-карта + считыватель) ~€120**
  в первый год с НДС и доставкой, продление ~€30/год. Уточнять у Certum перед покупкой.
...
3. **Сертификат для подписи** — блокирующий для `uiAccess`, а без `uiAccess` UIPI молча
   ...
   (Certum Open Source: облачный ~€60 / аппаратный ~€120 первый год, ~€30 далее, физлицам,
   проект открытый — так подписан OptiKey; либо Azure Artifact Signing $9.99/мес, ограничен
   по странам). Цены у источников расходятся — уточнять перед покупкой. Отдельный
   практический фактор — возможность оплаты и доставки в нашу страну.
